...
def crea_config_chart(title: str, dataset: pd.DataFrame, selected_cols: list) -> dict:
    ...
    return {
        ...
        "legend": {"data": selected_cols[1:], "bottom": "10%"},
        "xAxis": {"type": "category"},
        "yAxis": {},
        "grid": {"bottom": "20"},
        "series": [{
...
def display_temporal_tab(container, df: pd.DataFrame):
    """
    Visualizza i grafici temporali in un layout a schede (card layout) con lazy loading e animazioni.
    """
    ...
    # Si assume che la colonna "data" sia formattata come stringa
    ...
    # L'ordine delle colonne, in base alla funzione di preparazione, è:
    ...
    # (Opzionale) CSS per animazioni di transizione
    st.markdown("""
        <style>
            .echarts-container { 
